...
""")
        ...
        self.parser.add_argument("-p", "--port", required=False, type=int, help="Enter Target Port.")
        ...
        self.run(module=self.module, target=self.target, port=self.port ,username=self.user, wordlist=self.wordlist, timeout=self.timeout)

    def run(self, *c, module : str, target : str, port : int, username : str, wordlist : str, timeout : int = 3) -> None:
        match module.lower():
            case "ftp":
            ...
            case "ssh":
                if target != "" and username != "" and wordlist != "":
                    SSH_BRUTEFORCE(target=target, port=port, username=username, timeout=timeout, wordlist=wordlist).start()
